		...
		if LocalPlayer["state"]["Active"] and Display then
			...
			if IsPedInAnyVehicle(Ped) then
				...
				if Engine ~= VEngine then
					-- EM CASO DE UM '/fix' O MOTOR DEIXA DE ENTRAR NA CONDIÇÃO DE REGREÇÃO E VOLTA AO NORMAL
					if VEngine > 105 then
						VehicleBroken = false
					end

					-- SE O MOTOR FOR MENOR OU IGUAL A '100.00' (10%) COMEÇA UMA CONTAGEM REGRESSIVA PRO CARRO PARAR TOTALMENTE
					-- SEM ESSA CONDIÇÃO O CARRO NÃO PARA NUNCA E FICA NOS 10% DE MOTOR MESMO TORNANDO A BATER O CARRO
					if VEngine <= 100 and VehicleBroken == false then
					...
					-- PARA O CARRO TOTALMENTE E O DEIXA INUTILIZÁVEL ENQUANTO NÃO FOR CONCERTADO
					if VehicleBroken == true then
						SetVehicleEngineOn(Vehicle, false, true, true)
						SetVehicleUndriveable(Vehicle, true)

						SendNUIMessage({ Action = "Engine", Number = Engine })
					else -- ESSE 'else' É A CONDIÇÃO PADRÃO, QUE É APENAS RETORNAR O VALOR DA VIDA DO MOTOR PARA O '.js'
...
RegisterCommand("Beltz", function(source)
	...
	if IsPedInAnyVehicle(Ped) then
		if not IsPedOnAnyBike(Ped) and not IsPedInAnyHeli(Ped) and not IsPedInAnyPlane(Ped) then
			if SeatbeltLock then
				TriggerEvent("sounds:source", "unbelt", 0.4)
				SendNUIMessage({ Action = "Seatbelt", Status = false })
				SeatbeltLock = false
			else
				TriggerEvent("sounds:source", "belt", 0.4)
